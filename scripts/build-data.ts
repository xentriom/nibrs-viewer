...
const GENERATED_OPTIONS_PATH = path.join(ROOT, "lib", "nuqs-options.generated.ts");
...
function writeGeneratedOptions(options: { agencyIds: string[]; offenseCodes: string[] }) {
  const { agencyIds, offenseCodes } = options;
  const yearOptions = `[${YEARS.join(", ")}] as const`;
  const monthOptions = `["all", ${Array.from({ length: 12 }, (_, i) => `"${i + 1}"`).join(", ")}] as const`;
  const agencyOptions = `["all", ${agencyIds.map((id) => JSON.stringify(id)).join(", ")}] as const`;
  const offenseCodeOptions = `["all", ${offenseCodes.map((c) => JSON.stringify(c)).join(", ")}] as const`;

  const content = `// Auto-generated by scripts/build-data.ts — do not edit manually

export const yearOptions = ${yearOptions};
export const monthOptions = ${monthOptions};
export const agencyOptions = ${agencyOptions};
export const offenseCodeOptions = ${offenseCodeOptions};
`;

  return writeFile(GENERATED_OPTIONS_PATH, content, "utf8");
}
...
async function main() {
  ...
  for (const year of YEARS) {
    ...
    for (const a of dataset.agencyBreakdown) {
      agencyIds.add(a.agencyId);
    }
    for (const o of dataset.offenseBreakdown) {
      offenseCodes.add(o.offenseCode);
    }
  ...
  await writeGeneratedOptions({
    agencyIds: [...agencyIds].sort((a, b) => a.localeCompare(b, undefined, { numeric: true })),
    offenseCodes: [...offenseCodes].sort((a, b) =>
      a.localeCompare(b, undefined, { numeric: true }),
    ),
  });
  console.log(`Options generated: ${agencyIds.size} agencies, ${offenseCodes.size} offense codes`);
}
